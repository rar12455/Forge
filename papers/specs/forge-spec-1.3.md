...
`@uninit` opts out of zero initialization. Reading an `@uninit`
variable before a provable write is a compile error.
...
This is the primary mechanism for error handling. See §7.

```forge
// Pure function returning two values
...
// Procedure returning a value and an error code
...
Closures use `|parameters| { body }` syntax. No trailing arrow.
...
@mut uint32 count = 0;
@mut () counter = @mut || { count += 1uint32; };
```

Closures capturing by copy are always permitted. A closure that
writes to a captured variable must itself be `@mut`.
...
```forge
uint32 internal(uptr addr) { return addr as uint32 & 0xFFF; }
...
// Procedure that can fail and returns a value
...
Point q  = Point { y: 5, x: -3 };    // field order irrelevant
Point z  = Point {};                   // zero-initialized
Point p2 = Point { y: 99, ..p };      // struct update
```
...
containing all the return values in declaration order. For two values
where the total size is ≤ 16 bytes (the common case), both values are
returned in registers (`rax`, `rdx`) with no overhead.

```forge
// (uptr, ErrorCode) — 8 + 4 = 12 bytes — returned in rax:rdx
...
`forgec` produces ELF64 object files. Linked by `ld` or `lld`.
...
performs I/O. Verified statically. A procedure call from a pure
...
**No silent integer conversions.** Every integer type change
corresponds to an explicit `as` in the source.
...
*The Forge Programming Language Specification, Version 1.3*
